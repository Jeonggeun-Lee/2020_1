...
public:
  ...
  virtual void move(vector<vector<int> > &map, int maxx, int maxy)
  {
    ...
    if(nSleep==0)
    {
      ...
      srand(time(NULL));
      dir = rand()%9+1;
      if(dir==1) p += Point(-1,1);
      else if(dir==2) p += Point(0,1);
      else if(dir==3) p += Point(1,1);
      else if(dir==4) p += Point(-1,0);
      else if(dir==5) p += Point(0,0);
      else if(dir==6) p += Point(1,0);
      else if(dir==7) p += Point(-1,-1);
      else if(dir==8) p += Point(0,-1);
      else if(dir==9) p += Point(1,-1);
      clip(maxx, maxy);
    }
    eat(map);
  }
