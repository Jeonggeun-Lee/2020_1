...
public:
	...
	void move(vector<vector<int> > &map, int maxx, int maxy)
	{
		...
		if(nSleep==0)
		{
			...
			srand(time(NULL));
			dir = rand()%4+1;
			if(dir==1) p += Point(1,0);
			else if(dir==2) p += Point(-1,0);
			else if(dir==3) p += Point(0,1);
			else if(dir==4) p += Point(0,-1);
			clip(maxx, maxy);
		}
		eat(map);
	}
...
public:
	...
	void move(vector<vector<int> > &map, int maxx, int maxy)
	{
		...
		if(nSleep==0)
		{
			if( bHori == true )	p += Point((-1+(rand()%2)*2)*(1+rand()%2),0);
			else p += Point(0,(-1+(rand()%2)*2)*(1+rand()%2));
			clip(maxx,maxy);
		}
		eat(map);
	}
